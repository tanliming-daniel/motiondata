...
FRPC_CONFIG=${FRPC_CONFIG:-$SCRIPT_DIR/frpc_api.ini}
LOCAL_PORT=${LOCAL_PORT:-7091}
REMOTE_PORT=${REMOTE_PORT:-7091}
LOG_FILE=${LOG_FILE:-$SCRIPT_DIR/frpc_api.log}
PID_FILE=${PID_FILE:-$SCRIPT_DIR/frpc_api.pid}
RUNTIME_CONFIG=${RUNTIME_CONFIG:-$SCRIPT_DIR/frpc_api.runtime.ini}
...
nohup "$FRPC_BIN" -c "$RUNTIME_CONFIG" > "$LOG_FILE" 2>&1 &
PID=$!
printf '%s\n' "$PID" > "$PID_FILE"
