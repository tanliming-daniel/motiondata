...
class CacheManifest:
    ...
    def record(self, path: Path, *, object_id: str, dataset: str, checksum: bool = False, hit: bool = False, commit: bool = True) -> None:
        if not path.is_file():
            return
        now = time.time()
    ...
    def reconcile(self) -> int:
        ...
        for path in self.root.rglob("*.glb"):
            ...
            self.record(path, object_id=str(row[0]) if row else "", dataset=str(row[1]) if row else path.parts[0], commit=False)
